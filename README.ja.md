...
**[⬇ OnlyLimits 1.1 をダウンロード (.dmg)](https://github.com/Glym143/onlylimits/releases/latest)**
...
| 🧠 **Mac のメモリも** | 使用中の RAM を制限の隣に表示 - メニューバーにはチップ + **使用中のギガバイト**、パネルにはバーで総容量・スワップ・macOS の**メモリ圧迫**（緑 / 黄 / 赤）。 |
| 💽 **…ディスク容量も** | 起動ボリュームも同じ流儀で - メニューバーにはドライブ記号 + **使用中の容量**、パネルにはバーで総容量と空き容量。色は*残り*で決まるので、本当に逼迫したときだけ警告色になります。 |
...
この 3 つの隣には 🧠 と 💽 の 2 つのボタンがあります。それぞれ、選んだモードの表示に**使用中の RAM（またはディスク容量）を追加**します - どのモードでも、ワンクリックでオン/オフ。パネル内のメモリ行とストレージ行は ⚙️ で個別に切り替えます。
...
- **ディスク容量**は `/` のボリュームリソース値から取得します - 容量と「available for important usage」（空き容量 *＋* macOS が必要に応じて解放できる分）で、Finder が表示するのと同じ数値です。RAM の 2 進 GiB とは違い、Finder と同じ 10 進 GB/TB で数えます。30 秒ごとにサンプリング。
- **Mac のメモリ**はカーネルから直接読み取ります（`host_statistics64` + `sysctl`、外部コマンドの実行なし）: *使用中 = アプリ + 確保済み + 圧縮*、アクティビティモニタとまったく同じ計算です（キャッシュされたファイルは「使用中」に含めません）。色は生の使用率ではなくカーネルの**メモリ圧迫**レベルに従います - キャッシュと圧縮ページで 90% に達している Mac はまったく健全です。2〜3 秒ごとにサンプリングし、表示上の数値が変わったときだけ描き直します。
